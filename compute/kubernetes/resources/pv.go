...
func DeletePV(ctx context.Context, taskID string, namespace string, client kubernetes.Interface, log *logger.Logger) error {
	...
	for i := range maxRetries {
		...
		// PVs are not bound to any namespace. Funnel labels them with a namespace so that we do not delete PVs that were not generated by this instance of Funnel.
		if pv.Labels["namespace"] != namespace {
			log.Warn("PV namespace label does not match expected namespace, skipping deletion",
				"pv", name, "expected", namespace, "actual", pv.Labels["namespace"])
			return nil
		}
